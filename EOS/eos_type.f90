...
contains
  ...
  recursive subroutine eos_vector_in(state, state_in)
    ...
    type is (eos_t)
       ...
       cptr = c_loc(state_in % rho)
       call c_f_pointer(cptr, state % rho, state % width)
       cptr = c_loc(state_in % T)
       call c_f_pointer(cptr, state % T, state % width)
       cptr = c_loc(state_in % p)
       call c_f_pointer(cptr, state % p, state % width)
       cptr = c_loc(state_in % e)
       call c_f_pointer(cptr, state % e, state % width)
       cptr = c_loc(state_in % h)
       call c_f_pointer(cptr, state % h, state % width)
       cptr = c_loc(state_in % s)
       call c_f_pointer(cptr, state % s, state % width)
       cptr = c_loc(state_in % dpdT)
       call c_f_pointer(cptr, state % dpdT, state % width)
       cptr = c_loc(state_in % dpdr)
       call c_f_pointer(cptr,  state % dpdr, state % width)
       cptr = c_loc(state_in % dedT)
       call c_f_pointer(cptr, state % dedT, state % width)
       cptr = c_loc(state_in % dedr)
       call c_f_pointer(cptr, state % dedr, state % width)
       cptr = c_loc(state_in % dedr)
       call c_f_pointer(cptr, state % dhdT, state % width)
       cptr = c_loc(state_in % dhdr)
       call c_f_pointer(cptr, state % dhdr, state % width)
       cptr = c_loc(state_in % dsdT)
       call c_f_pointer(cptr, state % dsdT, state % width)
       cptr = c_loc(state_in % dsdr)
       call c_f_pointer(cptr, state % dsdr, state % width)
       cptr = c_loc(state_in % dpde)
       call c_f_pointer(cptr, state % dpde, state % width)
       cptr = c_loc(state_in % dpdr_e)
       call c_f_pointer(cptr, state % dpdr_e, state % width)
       cptr = c_loc(state_in % xn(1))
       call c_f_pointer(cptr, state % xn, state % spec_width)
       cptr = c_loc(state_in % aux(1))
       call c_f_pointer(cptr, state % aux, state % aux_width)
       cptr = c_loc(state_in % cv)
       call c_f_pointer(cptr, state % cv, state % width)
       cptr = c_loc(state_in % cp)
       call c_f_pointer(cptr, state % cp, state % width)
       cptr = c_loc(state_in % xne)
       call c_f_pointer(cptr, state % xne, state % width)
       cptr = c_loc(state_in % xnp)
       call c_f_pointer(cptr, state % xnp, state % width)
       cptr = c_loc(state_in % eta)
       call c_f_pointer(cptr, state % eta, state % width)
       cptr = c_loc(state_in % pele)
       call c_f_pointer(cptr, state % pele, state % width)
       cptr = c_loc(state_in % ppos)
       call c_f_pointer(cptr, state % ppos, state % width)
       cptr = c_loc(state_in % mu)
       call c_f_pointer(cptr, state % mu, state % width)
       cptr = c_loc(state_in % mu_e)
       call c_f_pointer(cptr, state % mu_e, state % width)
       cptr = c_loc(state_in % y_e)
       call c_f_pointer(cptr, state % y_e, state % width)
       cptr = c_loc(state_in % dedX(1))
       call c_f_pointer(cptr, state % dedX, state % spec_width)
       cptr = c_loc(state_in % dpdX(1))
       call c_f_pointer(cptr, state % dpdX, state % spec_width)
       cptr = c_loc(state_in % dhdX(1))
       call c_f_pointer(cptr, state % dhdX, state % spec_width)
       cptr = c_loc(state_in % gam1)
       call c_f_pointer(cptr, state % gam1, state % width)
       cptr = c_loc(state_in % cs)
       call c_f_pointer(cptr, state % cs, state % width)
       cptr = c_loc(state_in % abar)
       call c_f_pointer(cptr, state % abar, state % width)
       cptr = c_loc(state_in % zbar)
       call c_f_pointer(cptr, state % zbar, state % width)
       cptr = c_loc(state_in % dpdA)
       call c_f_pointer(cptr, state % dpdA, state % width)
       cptr = c_loc(state_in % dpdZ)
       call c_f_pointer(cptr, state % dpdZ, state % width)
       cptr = c_loc(state_in % dedA)
       call c_f_pointer(cptr, state % dedA, state % width)
       cptr = c_loc(state_in % dedZ)
       call c_f_pointer(cptr, state % dedZ, state % width)
    ...
    type is (eos_t_1D)
       ...
       cptr = c_loc(state_in % rho(lo(1)))
       call c_f_pointer(cptr, state % rho, state % width)
       cptr = c_loc(state_in % T(lo(1)))
       call c_f_pointer(cptr, state % T, state % width)
       cptr = c_loc(state_in % p(lo(1)))
       call c_f_pointer(cptr, state % p, state % width)
       cptr = c_loc(state_in % e(lo(1)))
       call c_f_pointer(cptr, state % e, state % width)
       cptr = c_loc(state_in % h(lo(1)))
       call c_f_pointer(cptr, state % h, state % width)
       cptr = c_loc(state_in % s(lo(1)))
       call c_f_pointer(cptr, state % s, state % width)
       cptr = c_loc(state_in % dpdT(lo(1)))
       call c_f_pointer(cptr, state % dpdT, state % width)
       cptr = c_loc(state_in % dpdr(lo(1)))
       call c_f_pointer(cptr,  state % dpdr, state % width)
       cptr = c_loc(state_in % dedT(lo(1)))
       call c_f_pointer(cptr, state % dedT, state % width)
       cptr = c_loc(state_in % dedr(lo(1)))
       call c_f_pointer(cptr, state % dedr, state % width)
       cptr = c_loc(state_in % dedr(lo(1)))
       call c_f_pointer(cptr, state % dhdT, state % width)
       cptr = c_loc(state_in % dhdr(lo(1)))
       call c_f_pointer(cptr, state % dhdr, state % width)
       cptr = c_loc(state_in % dsdT(lo(1)))
       call c_f_pointer(cptr, state % dsdT, state % width)
       cptr = c_loc(state_in % dsdr(lo(1)))
       call c_f_pointer(cptr, state % dsdr, state % width)
       cptr = c_loc(state_in % dpde(lo(1)))
       call c_f_pointer(cptr, state % dpde, state % width)
       cptr = c_loc(state_in % dpdr_e(lo(1)))
       call c_f_pointer(cptr, state % dpdr_e, state % width)
       cptr = c_loc(state_in % xn(lo(1),1))
       call c_f_pointer(cptr, state % xn, state % spec_width)
       cptr = c_loc(state_in % aux(lo(1),1))
       call c_f_pointer(cptr, state % aux, state % aux_width)
       cptr = c_loc(state_in % cv(lo(1)))
       call c_f_pointer(cptr, state % cv, state % width)
       cptr = c_loc(state_in % cp(lo(1)))
       call c_f_pointer(cptr, state % cp, state % width)
       cptr = c_loc(state_in % xne(lo(1)))
       call c_f_pointer(cptr, state % xne, state % width)
       cptr = c_loc(state_in % xnp(lo(1)))
       call c_f_pointer(cptr, state % xnp, state % width)
       cptr = c_loc(state_in % eta(lo(1)))
       call c_f_pointer(cptr, state % eta, state % width)
       cptr = c_loc(state_in % pele(lo(1)))
       call c_f_pointer(cptr, state % pele, state % width)
       cptr = c_loc(state_in % ppos(lo(1)))
       call c_f_pointer(cptr, state % ppos, state % width)
       cptr = c_loc(state_in % mu(lo(1)))
       call c_f_pointer(cptr, state % mu, state % width)
       cptr = c_loc(state_in % mu_e(lo(1)))
       call c_f_pointer(cptr, state % mu_e, state % width)
       cptr = c_loc(state_in % y_e(lo(1)))
       call c_f_pointer(cptr, state % y_e, state % width)
       cptr = c_loc(state_in % dedX(lo(1),1))
       call c_f_pointer(cptr, state % dedX, state % spec_width)
       cptr = c_loc(state_in % dpdX(lo(1),1))
       call c_f_pointer(cptr, state % dpdX, state % spec_width)
       cptr = c_loc(state_in % dhdX(lo(1),1))
       call c_f_pointer(cptr, state % dhdX, state % spec_width)
       cptr = c_loc(state_in % gam1(lo(1)))
       call c_f_pointer(cptr, state % gam1, state % width)
       cptr = c_loc(state_in % cs(lo(1)))
       call c_f_pointer(cptr, state % cs, state % width)
       cptr = c_loc(state_in % abar(lo(1)))
       call c_f_pointer(cptr, state % abar, state % width)
       cptr = c_loc(state_in % zbar(lo(1)))
       call c_f_pointer(cptr, state % zbar, state % width)
       cptr = c_loc(state_in % dpdA(lo(1)))
       call c_f_pointer(cptr, state % dpdA, state % width)
       cptr = c_loc(state_in % dpdZ(lo(1)))
       call c_f_pointer(cptr, state % dpdZ, state % width)
       cptr = c_loc(state_in % dedA(lo(1)))
       call c_f_pointer(cptr, state % dedA, state % width)
       cptr = c_loc(state_in % dedZ(lo(1)))
       call c_f_pointer(cptr, state % dedZ, state % width)
    ...
    type is (eos_t_2D)
       ...
       cptr = c_loc(state_in % rho(lo(1),lo(2)))
       call c_f_pointer(cptr, state % rho, state % width)
       cptr = c_loc(state_in % T(lo(1),lo(2)))
       call c_f_pointer(cptr, state % T, state % width)
       cptr = c_loc(state_in % p(lo(1),lo(2)))
       call c_f_pointer(cptr, state % p, state % width)
       cptr = c_loc(state_in % e(lo(1),lo(2)))
       call c_f_pointer(cptr, state % e, state % width)
       cptr = c_loc(state_in % h(lo(1),lo(2)))
       call c_f_pointer(cptr, state % h, state % width)
       cptr = c_loc(state_in % s(lo(1),lo(2)))
       call c_f_pointer(cptr, state % s, state % width)
       cptr = c_loc(state_in % dpdT(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dpdT, state % width)
       cptr = c_loc(state_in % dpdr(lo(1),lo(2)))
       call c_f_pointer(cptr,  state % dpdr, state % width)
       cptr = c_loc(state_in % dedT(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dedT, state % width)
       cptr = c_loc(state_in % dedr(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dedr, state % width)
       cptr = c_loc(state_in % dedr(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dhdT, state % width)
       cptr = c_loc(state_in % dhdr(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dhdr, state % width)
       cptr = c_loc(state_in % dsdT(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dsdT, state % width)
       cptr = c_loc(state_in % dsdr(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dsdr, state % width)
       cptr = c_loc(state_in % dpde(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dpde, state % width)
       cptr = c_loc(state_in % dpdr_e(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dpdr_e, state % width)
       cptr = c_loc(state_in % xn(lo(1),lo(2),1))
       call c_f_pointer(cptr, state % xn, state % spec_width)
       cptr = c_loc(state_in % aux(lo(1),lo(2),1))
       call c_f_pointer(cptr, state % aux, state % aux_width)
       cptr = c_loc(state_in % cv(lo(1),lo(2)))
       call c_f_pointer(cptr, state % cv, state % width)
       cptr = c_loc(state_in % cp(lo(1),lo(2)))
       call c_f_pointer(cptr, state % cp, state % width)
       cptr = c_loc(state_in % xne(lo(1),lo(2)))
       call c_f_pointer(cptr, state % xne, state % width)
       cptr = c_loc(state_in % xnp(lo(1),lo(2)))
       call c_f_pointer(cptr, state % xnp, state % width)
       cptr = c_loc(state_in % eta(lo(1),lo(2)))
       call c_f_pointer(cptr, state % eta, state % width)
       cptr = c_loc(state_in % pele(lo(1),lo(2)))
       call c_f_pointer(cptr, state % pele, state % width)
       cptr = c_loc(state_in % ppos(lo(1),lo(2)))
       call c_f_pointer(cptr, state % ppos, state % width)
       cptr = c_loc(state_in % mu(lo(1),lo(2)))
       call c_f_pointer(cptr, state % mu, state % width)
       cptr = c_loc(state_in % mu_e(lo(1),lo(2)))
       call c_f_pointer(cptr, state % mu_e, state % width)
       cptr = c_loc(state_in % y_e(lo(1),lo(2)))
       call c_f_pointer(cptr, state % y_e, state % width)
       cptr = c_loc(state_in % dedX(lo(1),lo(2),1))
       call c_f_pointer(cptr, state % dedX, state % spec_width)
       cptr = c_loc(state_in % dpdX(lo(1),lo(2),1))
       call c_f_pointer(cptr, state % dpdX, state % spec_width)
       cptr = c_loc(state_in % dhdX(lo(1),lo(2),1))
       call c_f_pointer(cptr, state % dhdX, state % spec_width)
       cptr = c_loc(state_in % gam1(lo(1),lo(2)))
       call c_f_pointer(cptr, state % gam1, state % width)
       cptr = c_loc(state_in % cs(lo(1),lo(2)))
       call c_f_pointer(cptr, state % cs, state % width)
       cptr = c_loc(state_in % abar(lo(1),lo(2)))
       call c_f_pointer(cptr, state % abar, state % width)
       cptr = c_loc(state_in % zbar(lo(1),lo(2)))
       call c_f_pointer(cptr, state % zbar, state % width)
       cptr = c_loc(state_in % dpdA(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dpdA, state % width)
       cptr = c_loc(state_in % dpdZ(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dpdZ, state % width)
       cptr = c_loc(state_in % dedA(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dedA, state % width)
       cptr = c_loc(state_in % dedZ(lo(1),lo(2)))
       call c_f_pointer(cptr, state % dedZ, state % width)
    ...
    type is (eos_t_3D)
       ...
       cptr = c_loc(state_in % rho(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % rho, state % width)
       cptr = c_loc(state_in % T(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % T, state % width)
       cptr = c_loc(state_in % p(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % p, state % width)
       cptr = c_loc(state_in % e(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % e, state % width)
       cptr = c_loc(state_in % h(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % h, state % width)
       cptr = c_loc(state_in % s(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % s, state % width)
       cptr = c_loc(state_in % dpdT(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dpdT, state % width)
       cptr = c_loc(state_in % dpdr(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr,  state % dpdr, state % width)
       cptr = c_loc(state_in % dedT(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dedT, state % width)
       cptr = c_loc(state_in % dedr(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dedr, state % width)
       cptr = c_loc(state_in % dedr(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dhdT, state % width)
       cptr = c_loc(state_in % dhdr(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dhdr, state % width)
       cptr = c_loc(state_in % dsdT(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dsdT, state % width)
       cptr = c_loc(state_in % dsdr(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dsdr, state % width)
       cptr = c_loc(state_in % dpde(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dpde, state % width)
       cptr = c_loc(state_in % dpdr_e(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dpdr_e, state % width)
       cptr = c_loc(state_in % xn(lo(1),lo(2),lo(3),1))
       call c_f_pointer(cptr, state % xn, state % spec_width)
       cptr = c_loc(state_in % aux(lo(1),lo(2),lo(3),1))
       call c_f_pointer(cptr, state % aux, state % aux_width)
       cptr = c_loc(state_in % cv(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % cv, state % width)
       cptr = c_loc(state_in % cp(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % cp, state % width)
       cptr = c_loc(state_in % xne(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % xne, state % width)
       cptr = c_loc(state_in % xnp(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % xnp, state % width)
       cptr = c_loc(state_in % eta(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % eta, state % width)
       cptr = c_loc(state_in % pele(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % pele, state % width)
       cptr = c_loc(state_in % ppos(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % ppos, state % width)
       cptr = c_loc(state_in % mu(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % mu, state % width)
       cptr = c_loc(state_in % mu_e(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % mu_e, state % width)
       cptr = c_loc(state_in % y_e(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % y_e, state % width)
       cptr = c_loc(state_in % dedX(lo(1),lo(2),lo(3),1))
       call c_f_pointer(cptr, state % dedX, state % spec_width)
       cptr = c_loc(state_in % dpdX(lo(1),lo(2),lo(3),1))
       call c_f_pointer(cptr, state % dpdX, state % spec_width)
       cptr = c_loc(state_in % dhdX(lo(1),lo(2),lo(3),1))
       call c_f_pointer(cptr, state % dhdX, state % spec_width)
       cptr = c_loc(state_in % gam1(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % gam1, state % width)
       cptr = c_loc(state_in % cs(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % cs, state % width)
       cptr = c_loc(state_in % abar(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % abar, state % width)
       cptr = c_loc(state_in % zbar(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % zbar, state % width)
       cptr = c_loc(state_in % dpdA(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dpdA, state % width)
       cptr = c_loc(state_in % dpdZ(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dpdZ, state % width)
       cptr = c_loc(state_in % dedA(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dedA, state % width)
       cptr = c_loc(state_in % dedZ(lo(1),lo(2),lo(3)))
       call c_f_pointer(cptr, state % dedZ, state % width)
