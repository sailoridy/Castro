module tagging_module
  ...
  real(rt), allocatable, save ::    denerr,   dengrad, dengrad_rel
  real(rt), allocatable, save ::    enterr,   entgrad, entgrad_rel
  real(rt), allocatable, save ::    velerr,   velgrad, velgrad_rel
  real(rt), allocatable, save ::   temperr,  tempgrad, tempgrad_rel
  real(rt), allocatable, save ::  presserr, pressgrad, pressgrad_rel
  real(rt), allocatable, save ::    raderr,   radgrad, radgrad_rel
  integer,  allocatable, save ::  max_denerr_lev,   max_dengrad_lev, max_dengrad_rel_lev
  integer,  allocatable, save ::  max_enterr_lev,   max_entgrad_lev, max_entgrad_rel_lev
  integer,  allocatable, save ::  max_velerr_lev,   max_velgrad_lev, max_velgrad_rel_lev
  integer,  allocatable, save ::  max_temperr_lev,  max_tempgrad_lev, max_tempgrad_rel_lev
  integer,  allocatable, save ::  max_presserr_lev, max_pressgrad_lev, max_pressgrad_rel_lev
  integer,  allocatable, save ::  max_raderr_lev,   max_radgrad_lev, max_radgrad_rel_lev
...
contains
  ...
  subroutine laplac_error(tag,taglo,taghi, &
                          set,clear, &
                          var,varlo,varhi, &
                          lo,hi,nd, &
                          level) &
                          bind(C, name="laplac_error")
  ...
  subroutine denerror(tag,taglo,taghi, &
                      set,clear, &
                      den,denlo,denhi, &
                      lo,hi,nd, &
                      level) &
  ...
  subroutine temperror(tag,taglo,taghi, &
                       set,clear, &
                       temp,templo,temphi, &
                       lo,hi,np, &
                       level) &
                       bind(C, name="temperror")
  ...
  end subroutine temperror

  ! ::: -----------------------------------------------------------
  ! ::: This routine will tag high error cells based on the pressure
  ! ::: -----------------------------------------------------------

  subroutine presserror(tag,taglo,taghi, &
                        set,clear, &
                        press,presslo,presshi, &
                        lo,hi,np, &
                        level) &
                        bind(C, name="presserror")
  ...
  end subroutine presserror

  ! ::: -----------------------------------------------------------
  ! ::: This routine will tag high error cells based on the velocity
  ! ::: -----------------------------------------------------------

  subroutine velerror(tag,taglo,taghi, &
                      set,clear, &
                      vel,vello,velhi, &
                      lo,hi,nv, &
                      level) &
                      bind(C, name="velerror")
    ...
    integer, intent(in), value :: set, clear, nv, level
    integer, intent(in) :: taglo(3), taghi(3)
  ...
  end subroutine velerror

  ! ::: -----------------------------------------------------------
  ! ::: This routine will tag high error cells based on the radiation
  ! ::: -----------------------------------------------------------

  subroutine raderror(tag,taglo,taghi, &
                      set,clear, &
                      rad,radlo,radhi, &
                      lo,hi,nr, &
                      level) &
                      bind(C, name="raderror")
    ...
    integer, intent(in), value :: set, clear, nr, level
    integer, intent(in) :: taglo(3), taghi(3)
  ...
  end subroutine raderror

  ! ::: -----------------------------------------------------------
  ! ::: This routine will tag high error cells based on the entropy
  ! ::: -----------------------------------------------------------

  subroutine enterror(tag,taglo,taghi, &
                      set,clear, &
                      ent,entlo,enthi, &
                      lo,hi,nr, &
                      level) &
                      bind(C, name="enterror")
  ...
  subroutine nucerror(tag,taglo,taghi, &
                      set,clear, &
                      t,tlo,thi, &
                      lo,hi,nr, &
                      level) &
                      bind(C, name="nucerror")
